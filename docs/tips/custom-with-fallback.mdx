...
import QueryBuilder, { Field, RuleGroupType, ValueEditor, ValueEditorProps } from "react-querybuilder";
...
<QueryBuilder
  fields={[{ name: "field1", label: "Field 1", valueEditorType: "select" },{ name: "field2", label: "Field 2" }]}
  query={{ combinator: "and", rules: [{field: "field1", operator: "=", value: "item2"}, {field: "field2", operator: "=", value: ""} ] }}
  controlElements={{ valueEditor: (props) => {
    if (props.type === "select") {
      // For all 'select' types, use custom logic:
      return (
        <select
