...
Adds a sub-group to the current group. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesAndAddersProps` interface:
...
Adds a rule to the current group. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesAndAddersProps` interface:
...
Clones the current group. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesProps` interface:
...
Clones the current rule. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionProps` interface:
...
Selects the `combinator` property for the current group, or the current independent combinator value. Default is [`ValueSelector`](./valueselector). Receives the following props per the `CombinatorSelectorProps` interface:
...
Provides a draggable handle for reordering rules and groups. Default is [`DragHandle`](./draghandle). Only rendered if [drag-and-drop is enabled](#enabledraganddrop). Note that this component must be based on `React.forwardRef`. Receives the forwarded `ref` and the following props per the `DragHandleProps` interface:
...
Selects the `field` property for the current rule. Default is [`ValueSelector`](./valueselector). Receives the following props per the `FieldSelectorProps` interface:
...
A small wrapper around the [`combinatorSelector`](#combinatorselector) component. Default is [`InlineCombinator`](./inlinecombinator). Receives the following props per the `InlineCombinatorProps` interface, which extends `CombinatorSelectorProps`:
...
Locks the current group (sets the `disabled` property to `true`). Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesProps` interface:
...
Locks the current rule (sets the `disabled` property to `true`). Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesProps` interface:
...
Toggles the `not` property of the current group between `true` and `false`. Default is [`NotToggle`](./nottoggle). Receives the following props per the `NotToggleProps` interface:
...
Selects the `operator` property for the current rule. Default is [`ValueSelector`](./valueselector). Receives the following props per the `OperatorSelectorProps` interface:
...
Removes the current group from its parent group's `rules` array. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionWithRulesProps` interface:
...
Removes the current rule from its parent group's `rules` array. Default is [`ActionElement`](./actionelement). Receives the following props per the `ActionProps` interface:
...
Rule layout component. Default is [`Rule`](./rule). Receives the following props per the `RuleProps` interface:
...
Rule group layout component. Default is [`RuleGroup`](./rulegroup). Receives the following props per the `RuleGroupProps` interface:
...
Shifts the current rule/group up or down in the query hierarchy. Default is [`ShiftActions`](./shiftactions). Receives the following props per the `ShiftActionsProps` interface:
...
Updates the `value` property for the current rule. Default is [`ValueEditor`](./valueeditor). Receives the following props per the `ValueEditorProps` interface:
...
Selects the `valueSource` property for the current rule. Default is [`ValueSelector`](./valueselector). Receives the following props per the `ValueSourceSelectorProps` interface:
...
Stores `value` as a `number` instead of a `string` whenever possible. When using one of the `"*-limited"` options, values will only be checked for numericity when the value editor's `inputType` evaluates to `"number"`.

> _Tip: Try the **`"enhanced-limited"`** option first._

- `true` and `"enhanced"`/`"enhanced-limited"` use [`numeric-quantity`](https://www.npmjs.com/package/numeric-quantity) with default options to determine numericity.
- `"native"`/`"native-limited"` use JavaScript's native `parseFloat` method to determine numericity.
- `"strict"`/`"strict-limited"` use `numeric-quantity` with the `allowTrailingInvalid` option set to `false`. Values must be numeric _in their entirety_ to be considered numeric, not just _start_ with a number as with `parseFloat`. See also [the corresponding `formatQuery` option](../utils/export#parse-numbers).
