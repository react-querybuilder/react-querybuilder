...
For more information about option list props like `fields`, see [Working with option lists](../tips/option-lists).
...
For more information about option list props like `operators`, see [Working with option lists](../tips/option-lists).
...
For more information about option list props like `combinators`, see [Working with option lists](../tips/option-lists).
...
| `ungroup`        | each <code>&lt;button&gt;</code> that ungroups a group                                     |
...
Returns the named query parameters made available to rules whose `valueSource` is `"parameter"` (see [the `"parameter"` value source](./valueeditor#the-parameter-value-source)). When the returned list is non-empty, the value editor renders a `<select>` of these parameter names (a multiselect for `in`/`notIn` operators); when it is nullish or empty, the editor renders a free-form text input. Defaults to an empty list.

Specify names **without** the parameter prefix (e.g. `() => [{ name: 'p1', label: 'Param 1' }]`); [`formatQuery`](../utils/export) adds the dialect-appropriate prefix.

`fieldData` provides the complete `Field` object for accessing custom properties.

:::tip

See the [external parameter manager](../tips/parameter-manager) example for a working demonstration of feeding an external component's state into `getParameters`.
...
For more information about option list props, see [Working with option lists](../tips/option-lists).
...
`boolean` (default `false`) _<DemoLink option="showUngroupButtons" />_
...
`boolean` (default `false`) _<DemoLink option="showUndoRedo" />_

Displays "undo"/"redo" buttons at the end of the outermost group's header.

Defaults to `true` for any query builder descending from [`QueryBuilderHistory`](../tips/undo-redo) (pass `showUndoRedo={false}` on either component to opt out), mirroring how [`enableDragAndDrop`](#enabledraganddrop) defaults to `true` under `QueryBuilderDnD`.

:::caution

This prop has no effect on its own, because the buttons it displays come from the [`undoRedoActions`](./querybuilder-controlelements#undoredoactions) control element, which defaults to `null`. Provide one of the following:

- `QueryBuilderHistory` from [`react-querybuilder/history`](../tips/undo-redo) higher in the component tree, which supplies the default [`UndoRedoActions`](./undoredoactions) and records the history it navigates. This is the recommended approach.
...
Configure button labels via `translations.undo` and `translations.redo`. See [Undo/redo](../tips/undo-redo) for the full guide.
...
Identifier for this query builder instance within React Query Builder's internal Redux store. Useful for addressing a specific query builder from outside its own component tree, e.g. an external toolbar or a keyboard shortcut handler (see [Undo/redo](../tips/undo-redo)).
...
If another _mounted_ query builder is already using the same `qbId`, this query builder falls back to a generated identifier and logs an error in non-production modes. Reusing a `qbId` _after_ the previous query builder has unmounted is supported (see [`preserveQueryStateOnUnmount`](#preservequerystateonunmount)).
