...
Like [`enableDragAndDrop`](#enabledraganddrop), this prop only works in conjunction with a context provider—in this case `QueryBuilderHistory` from [`react-querybuilder/history`](../tips/undo-redo), which both supplies the buttons and records the history they navigate. Setting `showUndoRedo` without it logs an error and renders nothing.
