...
In addition to `"value"` and `"field"`, the `valueSource` property accepts `"parameter"`. This lets a rule compare a field against a named query parameter (a placeholder resolved at execution time) rather than a literal value or another field.

To enable it, include `"parameter"` in a field's `valueSources` (or the `getValueSources` prop):

```tsx
const fields: Field[] = [{ name: 'f1', label: 'f1', valueSources: ['value', 'parameter'] }];
```

When the rule's `valueSource` is `"parameter"`, the value editor's behavior depends on the [`getParameters` prop](./querybuilder#getparameters):
...
Specify parameter names **without** the parameter prefix (e.g. `p1`, not `:p1`). [`formatQuery`](../utils/export) adds the appropriate prefix (`:`, `@`, `$`, etc.) for the target dialect. If a stored value already includes the prefix, it is left as-is.

:::

The selected parameter name is stored directly in the rule's `value`:

```json
{ "field": "f1", "operator": "=", "value": "p1", "valueSource": "parameter" }
```

#### Formatting parameter rules

[`formatQuery`](../utils/export) understands the `"parameter"` value source across output formats:
...
- **`parameterized_named`** — the name is registered as a key in `params` (with a `null` placeholder value, to be supplied at execution time).
...
The [external parameter manager](../tips/parameter-manager) example shows how to manage a list of named parameters outside the query builder and merge their values into a `"parameterized_named"` export.
