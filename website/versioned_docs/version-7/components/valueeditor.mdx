...
Since the field's `valueSources` property is an array with length > 1, a ["value source selector" component](./querybuilder-controlelements#valuesourceselector) is displayed to the left of the `<ValueEditor />`.
