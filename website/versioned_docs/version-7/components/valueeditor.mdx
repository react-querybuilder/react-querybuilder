...
When the rule's `valueSource` is `"parameter"`, the value editor's behavior depends on the [`parameters` prop](./querybuilder#parameters):

- If `parameters` is provided (a non-empty option list), the editor renders a `<select>` of the available parameter names.
