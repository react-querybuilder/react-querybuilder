...
<summary>
  <code>/debug</code> import <em>(deprecated)</em>
</summary>
...
The new [`insert`](./utils/query-management#insert) utility inserts rules or groups at any position in the query hierarchy, unlike [`add`](./utils/query-management#add) which only appends. Use `replace: true` to replace existing items.
