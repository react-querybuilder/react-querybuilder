...
`expressionParserCEL` is the ready-to-use handler for [`parseCEL`](./utils/import#cel), bound to the built-in CEL inverse. It handles arithmetic, function-call, and `min`/`max` template operands:
...
`expressionParserSpEL` is the ready-to-use handler for [`parseSpEL`](./utils/import#spel), bound to the built-in SpEL inverse. It handles infix arithmetic operands:
