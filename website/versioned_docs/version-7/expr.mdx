...
`formatQuery` needs an expression-aware **rule processor** to serialize the `lhs`/`rhs` nodes; without one, `lhs` is ignored and the `"expression"` value source isn't handled. The package provides ready-to-use processors bound to the built-in serializers:
...
`getExpressionRuleProcessorParameterized` completes the trio for the `parameterized`/`parameterized_named` formats. Only build the processors for the formats you actually export.
