...
The full export surface of `@react-querybuilder/core` is locked by a test (`packages/core/src/__tests__/publicApi.test.ts`) that fails on both accidental removal _and_ accidental addition, so every change to the surface requires a deliberate changelog entry.
