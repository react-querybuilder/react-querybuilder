...
Both are exported from `@react-querybuilder/core/parseRAQB` (and `react-querybuilder/parseRAQB`). Neither adds a dependency on RAQB or immutable.js.
...
```ts
import { parseRAQBFields } from '@react-querybuilder/core/parseRAQB';
...
import { parseRAQB, parseRAQBFields } from 'react-querybuilder/parseRAQB';
...
The reverse conversion is available as a [`formatQuery`](../utils/export#react-awesome-query-builder) export format, which is useful for a phased migration where both query builders must read the same stored queries.

```ts
import { formatQuery } from 'react-querybuilder';
import { Utils } from '@react-awesome-query-builder/core';

const jsonTree = formatQuery(query, 'raqb');
const immutableTree = Utils.checkTree(Utils.loadTree(jsonTree), raqbConfig);
```

`formatRAQBFields`, the inverse of `parseRAQBFields`, is exported from the same sub-path as `parseRAQB`:

```ts
import { formatRAQBFields } from 'react-querybuilder/parseRAQB';

const raqbConfig = { ...BasicConfig, fields: formatRAQBFields(fields) };
```

RAQB-specific options are passed to `formatQuery` through `context`:
