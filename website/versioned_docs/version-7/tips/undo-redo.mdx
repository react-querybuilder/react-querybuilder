...
`QueryBuilderNative` from [`@react-querybuilder/native`](../compat) works the same way—wrap it in `QueryBuilderHistory` and the undo/redo controls render as React Native components (`NativeUndoRedoActions`, styled with the `undoRedoActions` key of the `styles` prop).
...
Without a `QueryBuilderHistory` ancestor, `showUndoRedo` has no effect in React Native.
