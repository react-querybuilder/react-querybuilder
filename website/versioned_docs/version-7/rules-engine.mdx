...
Try the interactive [Rules Engine demo](/demo/rules-engine) with a musician-themed example, including live execution against sample data.
...
- [`json-rules-engine`](#json-rules-engine) — an array of rules for [json-rules-engine](https://github.com/CacheControl/json-rules-engine), plus [supplemental operators](#additional-operators).
- [`native`](#native) — a dependency-free, in-process evaluator function.
- [`node-rules`](#node-rules) — live [node-rules](https://github.com/mithunsatheesh/node-rules) `Rule` objects.
- [`rulepilot`](#rulepilot) — a single [rulepilot](https://github.com/andrewbrg/rulepilot) `Rule` (single-outcome decisioning).

The desired format is passed as the second argument—either a bare string or an options object with a `format` property. All formats honor the [evaluation mode](#evaluation-modes).
...
| `between` / `notBetween`          | same                                        | Inclusive range test. Bounds may be an array (`[lo, hi]`) or a comma-separated string (`"lo,hi"`); numeric bounds are parsed and reordered ascending, mirroring [`formatQuery`](./utils/export). |
...
The `node-rules` format returns an array of live [node-rules](https://github.com/mithunsatheesh/node-rules) `Rule` objects—complete with `condition`/`consequence` functions—ready to pass to a `RuleEngine`. Antecedents are compiled to the same predicates used by the [`native`](#native) format, so evaluation needs no operator registration. Each fired condition's consequent is pushed onto `fact.events`, mirroring the `json-rules-engine` result shape.
...
- **Single result.** A rules engine whose nested or overlapping conditions would fire _multiple_ consequents under the other formats yields only the first match. [Cumulative](#evaluation-modes) evaluation mode therefore has no rulepilot representation and **throws**.
...
- **`"cascade"`** (default): conditions are evaluated in order, like an `if`/`else-if`/`else` chain. A later sibling only fires when _all_ prior siblings' antecedents failed. This is achieved by AND-ing each exported rule with the negated antecedents of its prior siblings.
- **`"cumulative"`**: every condition is evaluated independently, so any number of them may fire. (The [`rulepilot`](#rulepilot) format returns a single result and therefore does not support this mode—it throws if requested.)

In both modes, nested conditions are guarded by their ancestor antecedents (structural nesting). A `defaultConsequent` behaves differently per mode: in `"cascade"` it is the `else` branch, firing only when _every_ sibling antecedent at its level fails; in `"cumulative"` it is an always-true baseline that fires on every run (still guarded by ancestor antecedents at nested levels).
...
Each property's `inputType` may be `'text'` (default), `'textarea'`, `'number'`, `'checkbox'`, or `'select'`. A `defaultValue` is seeded into `params[name]` when the type is first selected, and `'select'` inputs render the options listed in `values`. Selecting a different consequent type reseeds `params` from the new type's `defaultValue`s, or clears the managed `params` when the new type defines no `properties`.
...
The button titles and labels are customizable via the `shiftActionUp` and `shiftActionDown` [translations](#custom-translations). To intercept, cancel, or replace a shift, use the [`onMoveCondition`](#event-handlers) handler.
