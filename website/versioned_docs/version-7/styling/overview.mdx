...
For a "justified" layout, add the `queryBuilder-justified` class to the query builder using the [`controlClassnames` prop](../components/querybuilder#controlclassnames), or to any ancestor element.

Justified layout styles augment the default styles by pushing the "+ Rule", "+ Group", clone, lock, and remove buttons to the right edge of their respective containers. This can
...
- `dndCopy`: Assigned to a drop target when the cursor hovers over it while the "copy" modifier key is pressed (<kbd>Alt</kbd> on Windows/Linux, <kbd>⌥ Option</kbd> on macOS; see [Drag-and-drop § Cloning and grouping](../dnd#cloning-and-grouping) for more information)
