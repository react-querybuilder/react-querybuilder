...
description: Basic setup instructions and principles
...
Install `react-querybuilder` using [npm](https://www.npmjs.com/), [Yarn](https://yarnpkg.com/), [pnpm](https://pnpm.io/), or [Bun](https://bun.sh/), or follow [buildless setup instructions](./buildless).
...
<SandpackRQB rqbVersion={8} options={{ editorHeight: 160 }}>
...
<SandpackRQB rqbVersion={8} options={{ editorHeight: 240 }}>
...
Use the `formatQuery` function to convert query objects to formats like [SQL](https://en.wikipedia.org/wiki/SQL), [MongoDB](https://www.mongodb.com/), and [CEL](https://cel.dev) ([full documentation](./utils/export)). The example below shows real-time SQL generation—modify the query and watch the SQL update automatically.
