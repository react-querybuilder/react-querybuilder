...
export const fieldSelector = ({ value, options, path }) => (
  <span>
    {`${path[0] + 1}. `}
...
export const operatorSelector = ({ value, options }) => (
  <select value={value} disabled>
    {options.map(o => (
      <option key={o.name} value={o.name}>
        {o.label}
      ...
      <p>
        <label>
          <input
            ...
            onChange={e => {
              if (e.target.checked) {
                const q1 = update(query, 'value', toArray(query.rules[5].value), [5]);
                const q2 = update(q1, 'value', toArray(q1.rules[10].value), [10]);
                setQuery(update(q2, 'value', toArray(q2.rules[11].value), [11]));
              } else {
                const q1 = update(query, 'value', joinWith(query.rules[5].value, ','), [5]);
                const q2 = update(q1, 'value', joinWith(q1.rules[10].value, ','), [10]);
                setQuery(update(q2, 'value', joinWith(q2.rules[11].value), [11]));
              }
