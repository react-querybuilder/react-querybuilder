...
| Partial       | "sql"[^2], "parameterized"[^2], "drizzle"[^2], "elasticsearch"[^3]                            |
| None          | "parameterized_named"[^4], "prisma", "sequelize", "tanstack_db", "ldap"                       |
...
[^3]: ElasticSearch nested queries only support `"some"` and `"none"` match modes.

[^4]: PostgreSQL does not support named parameters
