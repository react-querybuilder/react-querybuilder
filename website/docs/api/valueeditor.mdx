...
When the `operator` for a rule is **"between"** or **"notBetween"**, two inputs will be displayed. Each input will have the class "rule-value-list-item" (see [CSS classes](./classnames)). They will manage the rule's `value` as a comma-separated string (e.g. `"value1,value2"`) unless [`listsAsArrays`](./querybuilder#listsasarrays) is `true`, in which case the `value` will be an actual `Array` (e.g. `["value1", "value2"]`).
