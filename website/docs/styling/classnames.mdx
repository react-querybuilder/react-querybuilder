...
- `.undoRedoActions` (the wrapper `<div>` around the ["undo"/"redo" buttons](../tips/undo-redo); the buttons themselves get `.undoRedoActions-undo` and `.undoRedoActions-redo`)
...
- `.dndPreviewPosition` (applied to elements at the preview position during [update-while-dragging](../dnd#updatewhiledragging))
- `.dndHidden` (applied to hide elements during [update-while-dragging](../dnd#updatewhiledragging))
