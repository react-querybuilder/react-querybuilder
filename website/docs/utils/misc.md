...
`(query: RuleGroupTypeAny, sourcePath: Path, targetPath: Path, options: GroupOptions) => RuleGroupTypeAny`

Creates a new group at the target path containing the existing rule/group at that path and the rule/group at the source path (in that order) in its `rules` array.
