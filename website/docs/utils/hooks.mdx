...
Used by the [`ShiftActions`](../components/shiftactions) component. Generates `shiftUp` and `shiftDown` methods used to move rules and group up or down, respectively, in the query hierarchy.
