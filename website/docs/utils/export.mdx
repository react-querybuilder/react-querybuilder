...
The `parameters` option supports rules whose `valueSource` is [`"parameter"`](../components/valueeditor#the-parameter-value-source). Provide the same option list passed to the [`parameters` prop](../components/querybuilder#parameters) (names without a prefix):
...
- **`parameterized_named`** — the name is registered as a `params` key with an `undefined` value (respecting `paramsKeepPrefix`), to be supplied at execution time.
