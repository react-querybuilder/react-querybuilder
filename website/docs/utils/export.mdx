...
The first argument is the `RuleType` object directly from the query. The second argument is of type `ValueProcessorOptions`. `parseNumbers` is copied directly from the `formatQuery` options (defaulting to `false`), and `escapeQuotes` is `true` or `false` when appropriate as determined by the internal `formatQuery` logic (generally speaking, quotes are escaped for string values and not escaped otherwise, as when values represent field names).

The default rule processors for each <abbr title="All formats except 'json' and 'json_without_ids'">query language format</abbr> are available as exports from `react-querybuilder`:

- `defaultValueProcessorByRule` for all SQL-based formats (no, "Value" is not a typo here)
...
For <abbr title="'sql', 'parameterized', and 'parameterized_named'">SQL-based formats</abbr>, `ruleProcessor` only replaces the "value" portion of the output (to the right of the operator). For other formats, the entire string or object representing the rule will be replaced by the return value of `ruleProcessor`.

`formatQuery` doesn't currently offer a way to customize the text of the entire rule for SQL-based formats.

:::

You can use the default rule processor as a fallback so your custom rule processor doesn't have to cover all cases, as shown below.
...
This option exists for backwards compatibility, but we recommend avoiding it for a few reasons. It has the same interface and behavior as `ruleProcessor` when accepting fewer than three arguments anyway, and using the legacy behavior (accepting three or more arguments) makes it more difficult to correctly fall back to a default processor since the options are not passed in.

:::

`valueProcessor` can be used in exactly the same way as `ruleProcessor`, as long as the function accepts fewer than three arguments. `ruleProcessor` was introduced in version 5 to clarify the behavior and provide a consistent function signature.
...
Notice that no options or additional properties of the rule are passed. Among other problems, this prevents `formatQuery` from setting the `escapeQuotes` option. This legacy behavior is documented here for completeness but, as stated above, is not recommended.
...
Versions of the default value processors using the newer `fn(rule, options)` signature as well as the legacy signature are available for all query language formats except "jsonlogic".
