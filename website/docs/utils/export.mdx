...
When querying across joined collections, fields can be prefixed with the collection alias to target a specific ref. Bare (unprefixed) fields resolve to the first collection in the `refs` object, or the first entry in `context.sourcePriority` if provided.

```ts
import * as tsdb from '@tanstack/db';

// Fields use dotted notation: "collectionAlias.fieldName"
const query = {
  combinator: 'and',
  rules: [
    { field: 'todo.completed', operator: '=', value: false },
    { field: 'lists.title', operator: '=', value: 'Work' },
  ],
};
...
const results = useLiveQuery(q =>
  q.from({ todo: todosCollection, lists: listsCollection }).where(where)
);
```

Use `context.sourcePriority` to control which collection bare (unprefixed) fields resolve to:

```ts
const where = formatQuery(query, {
  format: 'tanstack_db',
  context: {
    tanStackDbOperators: tsdb,
    // Bare fields like "title" resolve to `refs.lists` first
