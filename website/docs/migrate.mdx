...
The TypeScript interface for the main component's props object has been overhauled. The [`QueryBuilder`](./components/querybuilder) component will now automatically infer the query type as either `RuleGroupType` or `RuleGroupTypeIC` instead of relying on the now-deprecated (and ignored) `independentCombinators` prop. See [independent combinators](./components/querybuilder#independent-combinators).
