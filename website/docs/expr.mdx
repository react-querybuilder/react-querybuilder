...
The SQL export processor has an import-side mirror: a `getExpression` handler for [`parseSQL`](./utils/import#sql) that turns arithmetic/function operands back into expression nodes. `expressionParserSQL` is the ready-to-use handler bound to the built-in SQL inverse:
