...
`expressionParserSpEL` is the ready-to-use handler for [`parseSpEL`](./utils/import#spring-expression-language-spel), bound to the built-in SpEL inverse. It handles infix arithmetic operands:
...
The infix arithmetic operators (`+ - * / %`) map back to their `fn` keys (`add`/`subtract`/`multiply`/`divide`/`mod`); unknown operators drop the rule. To parse custom operations, build a handler with `getExpressionParserSpEL`, passing a partial inverse registry (arithmetic-operator token → `fn` key) and matching arity metadata:
...
parseSpEL('price > (cost * 2)', { getExpression });
```

:::note

Function and method call operands are **not** invertible on the SpEL import side. The core SpEL processor collapses method invocations, type references, and non-identifier compound nodes (`T(java.lang.Math).abs(cost)`, `name.toUpperCase()`, `pow(cost, 2)`) to invalid nodes with no children, so `abs`/`min`/`max`/`upper`/`lower` and custom functions cannot be recovered. Rules using them are dropped on import; a custom `operators` inverse can only override the five built-in arithmetic mappings.
