...
The `QueryBuilderDateTime` [context provider](./components/querybuilder#context) augments [`ValueEditor`](./components/valueeditor) with date/time functionality. Wrap your `QueryBuilder` with it to enable enhanced date/time editing for any field whose `inputType` is `"date"`/`"datetime-local"` or whose `datatype` begins with "date", "datetime", or "timestamp". Non-date fields fall back to the standard value editor.
...
`operatorModeController` derives the mode from the rule's **operator** instead of rendering a toggle. Register the relative operator(s) on your date fields with `withRelativeOperators`, then pass the controller to `QueryBuilderDateTime`:
...
const fieldsWithRelative = withRelativeOperators(fields);

<QueryBuilderDateTime modeController={operatorModeController}>
  <QueryBuilder fields={fieldsWithRelative} />
</QueryBuilderDateTime>;
```

`withRelativeOperators` appends a `"relative"` operator to each field's `operators` list (falling back to the core `defaultOperators` for fields without an explicit list). Selecting that operator puts the editor in relative mode; the value remains a `RelativeDateTimeValue`.

:::caution
The operators registered for mode-switching signal mode only. If you use a custom (non-standard) operator name, you must add matching [processor support](#materializing-relative-values) so the operator exports correctly. Standard comparison operators (`=`, `<`, etc.) work out of the box.
:::

#### Custom controllers

Supply your own object to fully control the UX. Use `createOperatorModeController` to build an operator-driven controller from a custom operator list or predicate, or write a controller from scratch:

```tsx
import { createOperatorModeController } from '@react-querybuilder/datetime';

// From a custom operator list:
const controller = createOperatorModeController(['relativeTo', 'within']);

// Or a predicate:
const controller = createOperatorModeController(op => op.startsWith('rel'));

// Fully custom (no built-in affordance; mode derived from value):
