...
Date/time fields support _relative_ values such as "three months ago" or "the beginning of this year" in addition to absolute (ISO 8601 string) values. The value editor presents an "Absolute"/"Relative" mode selector; in relative mode it shows an anchor selector, a signed offset, and a unit selector (the unit is hidden when the offset is `0`).
...
To use the relative-capable editor on its own (e.g. when providing custom `controlElements`), import `RelativeDateTimeValueEditor`:
