...
| Prop          | Type                                      | Description                                                                           |
| ------------- | ----------------------------------------- | ------------------------------------------------------------------------------------- |
| `labels`      | `{ shiftUp: string; shiftDown: string; }` | `translations['shiftActionUp'/'shiftActionDown'].label`, e.g. "▴"/"▾"                 |
| `titles`      | `{ shiftUp: string; shiftDown: string; }` | `translations['shiftActionUp'/'shiftActionDown'].title`, e.g. "Shift up"/"Shift down" |
| `className`   | `string`                                  | CSS `classNames` to be applied                                                        |
| `ruleOrGroup` | `RuleGroupTypeAny`                        | This rule or group                                                                    |
| `level`       | `number`                                  | The `level` of this rule or group                                                     |
| `context`     | `any`                                     | Container for custom props that are passed to all components                          |
| `validation`  | <code>boolean \| ValidationResult</code>  | Validation result of this rule or group                                               |
| `disabled`    | `boolean`                                 | Whether this rule or group is disabled/locked                                         |
| `path`        | `Path`                                    | [Path](../tips/path) of this rule or group                                            |
| `schema`      | `Schema`                                  | Query [schema](../typescript#miscellaneous)                                           |
| `lastInGroup` | `boolean`                                 | Whether this rule or group is the last of its siblings                                |
