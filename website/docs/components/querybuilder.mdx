...
Option list of named query parameters made available to rules whose `valueSource` is `"parameter"` (see [the `"parameter"` value source](./valueeditor#the-parameter-value-source)). When provided and non-empty, the value editor renders a `<select>` of these parameter names (a multiselect for `in`/`notIn` operators); when nullish or empty, it renders a free-form text input.
