---
title: QueryBuilder
description: The root component, context provider and wrapper div
---
...
Defaults to [`ActionElement`](./actionelement). The following props are passed:
...
Defaults to [`ValueSelector`](./valueselector). The following props are passed:
...
Defaults to [`DragHandle`](./draghandle). Note that this component must be based on `React.forwardRef`, and must always render an element (never return `null`). Aside from the forwarded `ref`, the following props are passed:
...
interface DragHandleProps {
  ...
  disabled: boolean; // Whether this rule/group is disabled
  path: number[];
  schema: Schema;
}
```
...
A small wrapper around the [`combinatorSelector`](#combinatorselector) component. The following props are passed:
...
interface ActionWithRulesProps {
  ...
  ruleOrGroup: RuleGroupTypeAny; // This group
  disabled: boolean; // Whether this group is disabled
  disabledTranslation: string; // translations.lockGroupDisabled, if parent group is not disabled
  path: number[];
  schema: Schema;
}
```
...
interface ActionWithRulesProps {
  ...
  ruleOrGroup: RuleType; // This rule
  disabled: boolean; // Whether this rule is disabled
  disabledTranslation: string; // translations.lockRuleDisabled, if parent group is not disabled
  path: number[];
  schema: Schema;
}
```
...
Defaults to [`NotToggle`](./nottoggle). The following props are passed:
...
Defaults to [`Rule`](./rule). The following props are passed:
...
Defaults to [`RuleGroup`](./rulegroup). The following props are passed:
...
Defaults to [`ValueEditor`](./valueeditor). The following props are passed:
...
interface ValueEditorProps {
  ...
  context: any; // Container for custom props that are passed to all components
  validation: boolean | ValidationResult; // validation result of this rule
  valueSource: 'field' | 'value'; // Value source for this rule
  listsAsArrays: boolean; // Whether to manage value lists (e.g. "between"/"in") as arrays
  parseNumbers: boolean; // Whether to parse real numbers from string values
  separator: ReactNode; // Separator element for series of editors (e.g. "between")
  disabled: boolean; // Whether this rule is disabled
  path: number[];
  schema: Schema;
}
...
This function should return the separator element for a given `field` and `operator`. The element can be any valid React element, including a bare string (e.g. "and" or "to") or an HTML element like `<span />`. It will be placed in between value editors when multiple are rendered, e.g. when the operator is "between".
...
Pass `true`, `"strict"`, or `"native"` to store `value` as an actual number when possible instead of a string. Passing `"native"` will use `parseFloat` to determine if a value is a number, while `true` or `"strict"` will use a more strict algorithm that requires the value to be numeric in its entirety (not just _start_ with a number as `parseFloat` requires). See more information in the note about the corresponding `formatQuery` option in the [Export documentation](../utils/export#parse-numbers).
