...
export default defineConfig(options => {
  ...
  const opts: Options[] = [
    ...
    // ESM, Webpack 4 support. Target ES2017 syntax to compile away optional chaining and spreads
    {
      ...commonOptions,
      entry: {
        'react-querybuilder_mantine.legacy-esm': 'src/index.ts',
      },
    ...
    // ESM for use in browsers. Minified, with `process` compiled away
    {
      ...
      ...productionOptions,
      entry: {
        'react-querybuilder_mantine.production': 'src/index.ts',
      },
      format: ['esm'],
      outExtension: () => ({ js: '.mjs' }),
    },
    // CJS development
    {
      ...commonOptions,
      entry: {
        'react-querybuilder_mantine.cjs.development': 'src/index.ts',
      },
      format: 'cjs',
      outDir: './dist/cjs/',
    },
    // CJS production
    {
      ...
      ...productionOptions,
      entry: {
        'react-querybuilder_mantine.cjs.production': 'src/index.ts',
      },
      format: 'cjs',
      outDir: './dist/cjs/',
      onSuccess: async () => {
