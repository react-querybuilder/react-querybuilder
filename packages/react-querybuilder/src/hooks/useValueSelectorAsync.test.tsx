...
describe('useValueSelectorAsync', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('rule and group context', () => {
    ...
    // TODO: This test is probably bad, but may be a bug
    // oxlint-disable-next-line no-disabled-tests
    it.skip('handles different values', async () => {
      const props = createDefaultProps({ value: 'currentValue', rule: createRule() });
      const props2 = { ...props, value: 'nextValue' };
      const params: UseValueSelectorAsyncParams = {
      ...
      const cached = selectCacheByKey(store.getState(), 'currentValue');
      const cached2 = selectCacheByKey(store.getState(), 'nextValue');
      expect(cached?.data).toEqual(props.options);
  ...
  describe('edge cases', () => {
    // TODO: What should this case actually do?
    ...
    // TODO: Test rejected promises
    // oxlint-disable-next-line no-disabled-tests
    it.skip('handles rejected promises', async () => {
      const props = createDefaultProps({ rule: createRule() });
      const params: UseValueSelectorAsyncParams = {
        loadOptionList: () => Promise.reject(Error('fake error')),
        getCacheKey: 'field',
      };
      const { store, wrapper } = getWrapper();
      ...
      const err = selectErrorByKey(store.getState(), 'testField');
