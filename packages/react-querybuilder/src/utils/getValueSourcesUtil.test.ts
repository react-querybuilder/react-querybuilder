...
const f2: F = toFullOption({ name: 'f2', label: 'F2', valueSources: ['field'] });
const f3: F = toFullOption({ name: 'f3', label: 'F3', valueSources: ['value', 'field'] });
const f4: F = toFullOption({ name: 'f4', label: 'F4', valueSources: ['field', 'value'] });
const f1f: F = toFullOption({ name: 'f1f', label: 'F1F', valueSources: () => ['value'] });
const f2f: F = toFullOption({ name: 'f2f', label: 'F2F', valueSources: () => ['field'] });
const f3f: F = toFullOption({
  name: 'f3f',
  label: 'F3F',
  valueSources: () => ['value', 'field'],
});
const f4f: F = toFullOption({
  name: 'f4f',
  label: 'F4F',
  valueSources: () => ['field', 'value'],
});
const f1fo: F = toFullOption({
  name: 'f1fo',
  label: 'F1FO',
  valueSources: (op: string) => (op === '=' ? ['value'] : ['value']),
});
const f2fo: F = toFullOption({
  name: 'f2fo',
  label: 'F2FO',
  valueSources: (op: string) => (op === '=' ? ['value'] : ['field']),
});
const f3fo: F = toFullOption({
  name: 'f3fo',
  label: 'F3FO',
  valueSources: (op: string) => (op === '=' ? ['value'] : ['value', 'field']),
});
const f4fo: F = toFullOption({
  name: 'f4fo',
  label: 'F4FO',
