...
const mongoQueryString = `{"$and":[{},{"firstName":null},{"lastName":{"$ne":null}},{"firstName":{"$in":["Test","This"]}},{"lastName":{"$nin":["Test","This"]}},{"firstName":{"$in":[]}},{"firstName":{"$gte":"Test","$lte":"This"}},{"firstName":{"$gte":"Test","$lte":"This"}},{"lastName":{"$lte":"Test","$gte":"This"}},{"firstName":{"$gte":"","$lte":""}},{"firstName":{"$gte":"","$lte":""}},{"age":{"$gte":12,"$lte":14}},{"age":"26"},{"isMusician":true},{"email":{"$regex":"@"}},{"email":{"$regex":"^ab"}},{"email":{"$regex":"com$"}},{"hello":{"$not":{"$regex":"com"}}},{"job":{"$not":{"$regex":"^Man"}}},{"job":{"$not":{"$regex":"ger$"}}},{"$or":[{"job":"Sales Executive"},{"job":{"$in":[]}},{"job":{"$gte":"just one value","$lte":""}}]}]}`;
const mongoQueryStringForValueSourceField =
  '{"$and":[{},{"firstName":null},{"lastName":{"$ne":null}},{"firstName":{"$in":["middleName","lastName"]}},{"lastName":{"$nin":["middleName","lastName"]}},{"firstName":{"$in":[]}},{"firstName":{"$gte":"middleName","$lte":"lastName"}},{"firstName":{"$gte":"middleName","$lte":"lastName"}},{"lastName":{"$lte":"middleName","$gte":"lastName"}},{"firstName":{"$gte":"","$lte":""}},{"firstName":{"$gte":"","$lte":""}},{"age":"iq"},{"isMusician":"isCreative"},{"$where":"this.email.includes(this.atSign)"},{"$where":"this.email.startsWith(this.name)"},{"$where":"this.email.endsWith(this.dotCom)"},{"$where":"!this.hello.includes(this.dotCom)"},{"$where":"!this.job.startsWith(this.noJob)"},{"$where":"!this.job.endsWith(this.noJob)"},{"$or":[{"job":"executiveJobName"}]}]}';
const celString =
...
it('formats to mongo query correctly - 3', () => {
  ...
  const mongoQueryString3 = EJSON.parse(
    `{"firstName":"Test","$or":[{"lastName":"Test2"},{"numField":{"$in":["1","2"]}}]}`
  );
  const formattedQuery = formatQuery(mongoQuery3, 'mongodb');
  expect(EJSON.parse(formattedQuery)).toEqual(mongoQueryString3);
});
...
describe('validation', () => {
  ...
  describe('mongodb', () => {
    it('should invalidate a mongodb query', () => {
      ...
      expect(formattedQueryMongoDb).toBe(
        `{"_id":"asd","ACC360Objecttype":"qwert","$or":[{"ACC360Objecttype":"orval"},{"ACC360Objecttype":"orval2"}],"$and":[{"ACC360Objecttype":"and111"},{"ACC360Objecttype":"and222"}]}`
      );
      expect(EJSON.parse(formattedQueryMongoDb)).toEqual(
        EJSON.parse(
          `{"_id":"asd","ACC360Objecttype":"qwert","$or":[{"ACC360Objecttype":"orval"},{"ACC360Objecttype":"orval2"}],"$and":[{"ACC360Objecttype":"and111"},{"ACC360Objecttype":"and222"}]}`
        )
      );
    ...
    it('should invalidate a jsonlogic rule', () => {
      expect(
        formatQuery(
          ...
          { format: 'jsonlogic', fields: [{ name: 'field', validator: () => false }] }
        )
...
describe('parseNumbers', () => {
  ...
  it('parses numbers for mongodb', () => {
    const formattedQuery = formatQuery(queryForNumberParsing, {
      format: 'mongodb',
      parseNumbers: true,
    });
    expect(EJSON.parse(formattedQuery)).toEqual(
      EJSON.parse(
        '{"$and":[{"f":"NaN"},{"f":0},{"f":0},{"f":0},{"$or":[{"f":1.5},{"f":1.5}]},{"f":{"$in":[0,1,2]}},{"f":{"$in":[0,1,2]}},{"f":{"$in":[0,"abc",2]}},{"f":{"$gte":0,"$lte":1}},{"f":{"$gte":0,"$lte":1}},{"f":{"$gte":0,"$lte":"abc"}},{"f":{"$gte":1,"$lte":""}},{"f":{"$gte":"","$lte":""}},{"f":{"$gte":1,"$lte":""}},{"f":{"$gte":{},"$lte":{}}}]}'
      )
    );
  });
  ...
  it('parses numbers for jsonlogic', () => {
    expect(formatQuery(queryForNumberParsing, { format: 'jsonlogic', parseNumbers: true })).toEqual(
      {
        and: [
          { '==': [{ var: 'f' }, 'NaN'] },
          { '==': [{ var: 'f' }, 0] },
