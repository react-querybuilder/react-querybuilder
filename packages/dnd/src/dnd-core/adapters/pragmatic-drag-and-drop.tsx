...
let pragmaticDragAndDrop: unknown = null;
...
async function loadPragmaticDragAndDrop() {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'test') {
      throw new Error(
        'Failed to load @atlaskit/pragmatic-drag-and-drop. Make sure @atlaskit/pragmatic-drag-and-drop is installed.'
      );
    }
...
async function initializePragmaticDragAndDropAdapter(): Promise<void> {
  if (!adapterLoaded) {
...
function useDrag(options: DragHookOptions): DragHookResult {
  if (!pragmaticDragAndDrop) {
    throw new Error(
      '@atlaskit/pragmatic-drag-and-drop not loaded. Ensure the adapter is initialized first.'
    );
  }
...
/**
 * Pragmatic Drag and Drop adapter instance
 */
export const pragmaticDragAndDropAdapter: DndAdapter = {
  useDrag,
  useDrop,
  DndProvider,
  isTouchDevice,
  initialize: initializePragmaticDragAndDropAdapter,
};
