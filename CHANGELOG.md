...
- Minor `QueryManager` behavior changes:
  - `subscribe` listeners now receive a `SubscriptionChange` object (`{ query, config }`, both booleans, at least one `true`) describing what changed. Zero-argument listeners (including `useSyncExternalStore`'s `onStoreChange`) are unaffected.
  - `reconfigure` inside a `batch` defers its notification into the batch's single notification (one `{ query: true, config: true }` instead of two). Options still apply immediately—`getConfigVersion()` increments right away and the change survives a rollback, as does the deferred notification.
  - `reconfigure` is now a no-op when the resolved configuration is unchanged: nothing re-derived, no version bump, no notification. Equality is structural for data, by identity for functions (see `optionsEqual` below), so options objects rebuilt each render no longer force a reconfigure—memoize callbacks to benefit. The gate compares the _merged_ options (or the replacement, under `{ replace: true }`).
  - `QueryManager` instances can now be wrapped in a `Proxy`. Internal state moved from `#private` fields (unreachable through a proxy, throwing `TypeError: Cannot read private member`) to a single symbol-keyed, non-enumerable own property. Public contract unchanged: `Object.keys()`, spread, and `JSON.stringify()` behave as before. The state bag is unexported, undocumented, and **not** a supported access path; its contents may change in any release. It also carries a non-enumerable `__v_skip` property so Vue's `reactive()` leaves manager internals unproxied—core's only framework-specific concession, and it avoids a Vue peer dependency.
