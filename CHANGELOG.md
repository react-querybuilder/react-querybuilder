...
- [#1065] New `"parameter"` value source, letting a rule compare a field against a named query parameter (placeholder) instead of a literal value or another field. Enable it by including `"parameter"` in a field's `valueSources` (or the `getValueSources` prop); the rule's `value` holds the selected parameter name.
  - New `QueryBuilder` prop `parameters` (a `FlexibleOptionList`). When provided and non-empty, the value editor renders a `<select>` of these parameter names (a multiselect for `in`/`notIn` operators); when nullish or empty, it falls back to a free-form text input. Names should be specified without a prefix (e.g. `[{ name: 'p1', label: 'Param 1' }]`).
