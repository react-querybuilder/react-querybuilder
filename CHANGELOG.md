...
- New `@react-querybuilder/expr` package adding arithmetic/function expression support to rules—either side of a rule can hold a recursive expression tree (e.g. `price * quantity`), though the bundled UI builds a full tree only on the right-hand side; on the left-hand side it wraps the field in a function, where the field is the function's first (governing) argument and any further arguments are edited as nested expressions.
  ...
  - `formatQuery` rule processors for the "sql", "parameterized"/"parameterized_named", and "jsonlogic" formats. Use the ready-made `expressionRuleProcessorSQL`/`expressionRuleProcessorParameterized`/`expressionRuleProcessorJsonLogic` bound to the built-ins, or build a custom processor with the `getExpressionRuleProcessorSQL`/`getExpressionRuleProcessorParameterized`/`getExpressionRuleProcessorJsonLogic` factories, each accepting a serializer registry merged over that format's defaults. Rules without expressions, or with an unsupported operator, fall back to the stock processor. The default JsonLogic serializers emit stock operators except for `abs`/`upper`/`lower`, whose runtime implementations are exported as `jsonLogicExpressionOperators` for registration on your `json-logic-js` instance.
