...
- `@react-querybuilder/core`: `QueryManager` instances can be wrapped in a `Proxy`. Its internal state previously lived in `#private` fields, which are unreachable through a proxy—inside a method invoked through one, `this` _is_ the proxy, while the private brand is on the target, so every access threw `TypeError: Cannot read private member`. Frameworks apply proxies routinely (Vue's `reactive()`, and by extension Vue Test Utils' handling of mount props), so this was hit by accident rather than by choice. State now lives in a single symbol-keyed, non-enumerable own property, which reads correctly through a proxy's `get` trap. The public contract is unchanged: `Object.keys()`, spread, and `JSON.stringify()` on a manager produce exactly what they did before. That state bag is **not** a supported access path—it is unexported and undocumented, and its contents may change in any release. It also carries a non-enumerable `__v_skip` property, which Vue's `reactive()` honors to leave the manager's internals unproxied; that is core's only framework-specific concession, and it avoids a Vue peer dependency.
