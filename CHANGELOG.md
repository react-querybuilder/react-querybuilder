...
- [#1078]/[#1079] [`QueryManager`](https://react-querybuilder.js.org/docs/utils/query-management#query-manager), a stateful, chainable wrapper around the [query tools](https://react-querybuilder.js.org/docs/utils/query-management#query-tools). Each method takes the same arguments as its query tool counterpart minus the leading `query` parameter. Adds `createRule`/`createRuleGroup` factories, `validate`, and `format`. Accepts the same `fields`, `operators`, `combinators`, and `getDefault*` configuration as the `QueryBuilder` component. No React dependency.
  ...
  - Rule configuration methods `getFieldData`, `getOperators`, `getValueSources`, `getMatchModes`, `getValues`, and `getValueEditorType`, plus `getRuleContext(pathOrID)` which resolves all of them for a single rule (along with its validation result) or returns `null` if the target isn't a rule. These share the new framework-agnostic `deriveRuleContext` utility with the `useRule` hook, so non-React implementations resolve a rule's configuration identically without reimplementing the precedence rules. (`QueryManager` has no `getInputType` option, so `inputType` reflects only a field's own property.)
  - Framework-agnostic classname utilities `deriveRuleClassNames`, `deriveRuleOuterClassName`, and `deriveRuleGroupClassNames`, now used by the `useRule` and `useRuleGroup` hooks. Composition is declared as data—each derived classname names its contributing `controlClassnames` keys and any state-dependent classes—so conditional elements like the rule wrapper and group header go through the same path as everything else and any implementation produces identical `class` attributes.
