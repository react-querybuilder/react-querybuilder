...
- [#1066] `parseMongoDB` gains a `getExpression` option for importing arithmetic/function expressions from `$expr` aggregation-expression operands, the inverse of the `mongodb_query` expression serializers. Supply the ready-made `expressionParserMongoDB` from `@react-querybuilder/expr` (or build one with the `getExpressionParserMongoDB` factory, which takes an inverse registry merged over `defaultMongoDBInverse`). An expression right-hand side becomes the rule's `value` with `valueSource: "expression"`; an expression left-hand side sets `field: ""` and stores the tree in `lhs`; inclusive-`between`/`notBetween` bounds become a two-element `value`. Only `$expr` operands are routed through the handler; bare field references and literals fall through to the stock logic. Unknown operators, arity mismatches, and unknown field leaves cause the rule to be dropped. A new `@react-querybuilder/core/parseMongoDB` subpath re-exports the parser internals so `@react-querybuilder/expr` can supply the handler without a runtime dependency cycle.
- [#1066] `parseCEL` gains a `getExpression` option for importing arithmetic/function expressions, the inverse of the `cel` expression serializers. Supply the ready-made `expressionParserCEL` from `@react-querybuilder/expr` (or build one with the `getExpressionParserCEL` factory, which takes an inverse registry merged over `defaultCELInverse`). An expression right-hand side becomes the rule's `value` with `valueSource: "expression"`; an expression left-hand side sets `field: ""` and stores the tree in `lhs`; inclusive-`between`/`notBetween` bounds become a two-element `value`. Infix arithmetic (`+ - * / %`) and the emulated `min`/`max` conditional templates are recovered; `abs`/`upper`/`lower` are not invertible (the CEL grammar cannot parse the constructs their exporters emit). Unknown operators/functions, arity mismatches, and unknown field leaves cause the rule to be dropped. A new `@react-querybuilder/core/parseCEL` subpath re-exports the parser internals so `@react-querybuilder/expr` can supply the handler without a runtime dependency cycle.
- [#1066] `parseSpEL` gains a `getExpression` option for importing arithmetic expressions, the inverse of the `spel` expression serializers. Supply the ready-made `expressionParserSpEL` from `@react-querybuilder/expr` (or build one with the `getExpressionParserSpEL` factory, which takes an inverse registry merged over `defaultSpELInverse`). An expression right-hand side becomes the rule's `value` with `valueSource: "expression"`; an expression left-hand side sets `field: ""` and stores the tree in `lhs`; inclusive-`between`/`notBetween` bounds become a two-element `value`. Only infix arithmetic (`+ - * / %`) is recovered; function and method calls (`abs`/`min`/`max`/`upper`/`lower` and custom functions) are not invertible—the core SpEL processor collapses method invocations, type references, and non-identifier compound nodes to childless invalid nodes—so a custom `operators` inverse can only override the five built-in arithmetic mappings. Unknown operators, arity mismatches, and unknown field leaves cause the rule to be dropped. A new `@react-querybuilder/core/parseSpEL` subpath re-exports the parser internals so `@react-querybuilder/expr` can supply the handler without a runtime dependency cycle.
