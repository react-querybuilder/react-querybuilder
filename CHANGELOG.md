...
- `@react-querybuilder/core`: `QueryManager#subscribe` listeners now receive a `SubscriptionChange` object (`{ query, config }`, both booleans, at least one `true`) describing what changed. Zero-argument listeners (including `useSyncExternalStore`'s `onStoreChange`) are unaffected.
- `@react-querybuilder/core`: `reconfigure` inside a `batch` defers its notification into the batch's single notification (one `{ query: true, config: true }` instead of two). Options still apply immediately—`getConfigVersion()` increments right away and the change survives a rollback, as does the deferred notification.
- `@react-querybuilder/core`: `QueryManager#reconfigure` is now a no-op when the resolved configuration is unchanged: nothing re-derived, no version bump, no notification. Equality is structural for data, by identity for functions (see `optionsEqual` below), so options objects rebuilt each render no longer force a reconfigure—memoize callbacks to benefit. The gate compares the _merged_ options (or the replacement, under `{ replace: true }`).
- `@react-querybuilder/core`: `QueryManager` instances can now be wrapped in a `Proxy`. Internal state moved from `#private` fields (unreachable through a proxy, throwing `TypeError: Cannot read private member`) to a single symbol-keyed, non-enumerable own property. Public contract unchanged: `Object.keys()`, spread, and `JSON.stringify()` behave as before. The state bag is unexported, undocumented, and **not** a supported access path; its contents may change in any release. It also carries a non-enumerable `__v_skip` property so Vue's `reactive()` leaves manager internals unproxied—core's only framework-specific concession, and it avoids a Vue peer dependency.

### Added

- New `@react-querybuilder/core` query management utilities
  - `optionsEqual(a, b)` reports whether two `QueryManagerOptions` objects describe the same configuration, and `valuesEqual(a, b)` is the underlying comparison—arrays and plain objects by value (descending into nested objects like `history` and `translations`), everything else, functions included, by identity. `QueryManager#reconfigure` uses it to gate itself; it is exported for adapters that want to decide whether to call `reconfigure` at all.
  - `SubscriptionChange`, the payload type passed to `QueryManager#subscribe` listeners.
  - `QueryManager#getRuleDefaultOperator(field)`, `QueryManager#getRuleDefaultValue(rule)`, and `QueryManager#getFieldMap()` expose derivations that were previously private.
  - `shouldCoalesce(prevSig, nextSig, prevAt, now, coalesceMs?)` is the history-coalescing predicate `QueryManager` applies internally. `QueryManager` now calls it rather than duplicating the rule.
  - `controlKeys`, `controlPropKeys`, and `controlKind` describe the query builder's controls as runtime data. `controlPropKeys` lists every prop each control receives. `controlKind` reports which bulk override (`actionElement`/`valueSelector`) applies to each control, replacing name-suffix sniffing such as `key.endsWith('Action')`. Compile-time gates in `react-querybuilder` fail the build if either drifts from the React prop types or from `defaultControlElements`.
  - `DefaultFieldProp` and `DefaultOperatorProp` name the union types of `QueryManagerOptions`' `getDefaultField`/`getDefaultOperator`, so callers building those values conditionally can annotate the intermediate instead of casting. The option fields are retyped to the aliases, which is structurally identical.
  - The query tools (`add`, `update`, `remove`, `move`, `insert`, `group`) and `createQueryActions` accept `freeze: false` to skip immer's auto-freeze on the returned query, and `QueryManagerOptions` accepts the option of the same name to skip every deep freeze the manager applies—the query, the field list, the field map, and the combinator list. Both default to `true`, so nothing changes unless you opt out. Intended for frameworks that wrap objects in proxies (Vue `reactive`, Solid stores), which cannot accept frozen input. The `*InPlace` tools never freeze, so the option is inert for them. The shallow copy returned by `QueryManager#getOptions()` is frozen either way. `setAutoFreeze` is re-exported from immer as a process-wide escape hatch for consumers mixing the query tools with their own `produce` calls.
  - A new `/derivations` subpath export (`@react-querybuilder/core/derivations`) provides the pure, framework-agnostic subset of the root surface: everything except `QueryManager` and `formatQuery` (the parsers already have subpaths of their own). Every binding is identical to the root's, so the subpath is purely a bundle-size guarantee for framework adapters that own their own state and never construct a manager. A post-build check gates the exclusion against the built output.
...
- [#1084] Optional peer dependencies no longer appear in published type declarations. Consumers who typecheck with `skipLibCheck: false` previously got `TS2307 Cannot find module 'drizzle-orm'` (and similar for `sequelize`, `@tanstack/db`, `json-rules-engine`, `node-rules`, and `rulepilot`) without installing packages they never used. The affected annotations now use local structural stand-ins or infer the library's types from caller-supplied arguments, so the exact types are still produced when the peer _is_ installed.
  ...
  - `@react-querybuilder/dnd`: `UseReactDnD`/`DndProp` and the `DndKitExports`/`PragmaticDndExports` adapter parameters are typed with new structural stand-ins (`ReactDnDExports`, `RdndBackendFactory`, etc.) instead of `typeof import('react-dnd')` and friends. The real exports of `react-dnd`, `react-dnd-html5-backend`, `react-dnd-touch-backend`, `@dnd-kit/core`, and `@atlaskit/pragmatic-drag-and-drop` are still assignable to them, but `UseReactDnD` now describes only the members the adapter actually uses rather than the entire `react-dnd` module.
...
- [#1078]/[#1079] [`QueryManager`](https://react-querybuilder.js.org/docs/utils/query-management#query-manager), a stateful, chainable wrapper around the [query tools](https://react-querybuilder.js.org/docs/utils/query-management#query-tools). Each method takes the same arguments as its query tool counterpart minus the leading `query` parameter. Accepts the same `fields`, `operators`, `combinators`, `getDefault*`, `resetOnFieldChange`/`resetOnOperatorChange`, `maxLevels`, `getInputType`, and `getSubQueryBuilderProps` configuration as the `QueryBuilder` component, plus `respectDisabled` (default `true`) and `queryDisabled`. No React dependency.
  - **State and factories**: `getQuery`/`setQuery`, `createRule`/`createRuleGroup`, `validate`, `format`, and `clone(options?)` (an independent manager with the same configuration and query, optionally regenerating every `id`).
  - **Subscriptions and batching**: `subscribe(listener)` returns an unsubscribe function; it and `getQuery` are bound to the instance, so both can be passed directly to `useSyncExternalStore`. `batch(fn)` defers history recording and notification until `fn` returns—one undo step, one notification—and restores the pre-batch state if `fn` throws. Batches may be nested; `undo`/`redo`/`clearHistory` may be called inside one, in which case the batch records no entry of its own.
  - **Undo/redo**: opt in with the `history` option (`true` or `{ maxHistory, coalesceMs }`) for `undo`, `redo`, `canUndo`, `canRedo`, `clearHistory`, and `getHistory`. Coalescing, `maxHistory` (default `50`), and `coalesceMs` (default `500`) match the `react-querybuilder/history` entry point; `defaultMaxHistory`, `defaultCoalesceMs`, and `QueryHistoryOptions` are exported from `@react-querybuilder/core` and shared by both.
  - **Strict mode**: opt-in `strict` throws a `QueryManagerError` (carrying a `code` and the full abort `info`) when a mutation's target can't be used, instead of failing silently. `onInvalidTarget` observes aborts without changing control flow. Both are configurable on the constructor and per call.
  - **Traversal and lookup**: `walk(options?)` is a generator yielding every rule and group—depth-first, pre-order, from the root—as `QueryNode` objects (`{ node, path, parent }`), with `WalkOptions` to restrict traversal to a subtree (`from`) and/or to rules or groups only (`rulesOnly`, `groupsOnly`). Also available as `rules()`, `groups()`, `find()`, `filter()`, and `[Symbol.iterator]()`, so a manager can be spread or used in `for...of`. The query-taking path utilities are methods minus their trailing `query` parameter (`findPath`, `findID`, `getPathOfID`, `pathIsDisabled`), joined by `getNode`, `getRule`, `getGroup`, and `getParent`, which accept a `Path` or an `id` and narrow the result, returning `null` when the target can't be resolved or is of the wrong kind. Lookups are backed by an `id`-to-`Path` index built once per query, so they are constant time rather than a tree walk; `validate()` results are cached the same way, meaning a custom `validator` with side effects may run fewer times than before.
  - **Configuration accessors**: `getFields` and `getCombinators` (the normalized top-level option lists, needed to populate selectors), `getFieldData`, `getOperators`, `getValueSources`, `getMatchModes`, `getValues`, and `getValueEditorType`. `getRuleContext(pathOrID)` resolves all of them for a single rule along with its `inputType`, `subQueryBuilderProps`, and validation result, and `getRuleGroupContext(pathOrID)` does the same for a group (combinator resolution, combinator-based classname, independent-combinator detection, and validation result—note that a group has no field-level validator fallback). Both return `null` when the target can't be resolved or is of the wrong kind.
  - **Inspection and conversion**: `isIC()`, `signatureOf(other)`, `diagnostics()` (shorthand for `format('diagnostics')`), and `toJSON()` (so `JSON.stringify(manager)` matches `JSON.stringify(manager.getQuery())`). `toIC()`/`fromIC()` return a _new_ manager with the query converted to or from the independent combinators structure, leaving the original unmodified, and `transform(options?)` runs `transformQuery` against the current query and returns its result directly, since the output may no longer be a valid query.
  - **`useQueryManager`** subscribes to a `QueryManager` and returns a `[query, manager]` tuple, re-rendering when the query changes. Pass an existing manager, or a query and options to have one created on the first render (and never recreated). Unlike the other hooks it does not use Redux and need not be rendered beneath a `QueryBuilder`, so it suits headless or fully custom interfaces; queries managed this way are correspondingly invisible to `useQueryBuilderQuery`, `useQueryBuilderSelector`, `useQueryBuilderHistory`, and `getDispatchQueryById`.
- [#1079] The logic behind the `QueryBuilder` component is now available as framework-agnostic utilities in `@react-querybuilder/core`, shared with the React hooks rather than duplicated by them. An implementation in another framework can reuse the precedence and policy rules instead of reimplementing them.
  - **Rule and group configuration**: `deriveRuleContext` and `deriveRuleGroupContext` (backing `useRule`/`useRuleGroup` and `QueryManager`'s context accessors), plus the precedence resolvers `resolveOperatorList`, `resolveDefaultOperator`, `resolveValueEditorType`, `resolveValueList`, and the `createRule`/`createRuleGroup` factories, all shared with `useQueryBuilderSetup`.
  - **Classnames**: `deriveRuleClassNames`, `deriveRuleGroupClassNames`, `deriveRuleOuterClassName`, `deriveRuleGroupOuterClassName`, and `deriveRuleClassName` (a single key). Composition is declared as data—each derived classname names its contributing `controlClassnames` keys and any state-dependent classes—so conditional elements go through the same path as everything else and any implementation produces identical `class` attributes. (A group's wrapper reflects fewer states than a rule's: `dndOver`, `dndCopy`, `dndDropNotAllowed`, and `hasSubQuery` are rule-only, so the two condition sets are declared separately.)
  - **Value editors**: `getValueEditorReset`, `getMultiValueUpdate`, `coerceBigIntValue`, `coerceInputType`, and `isBetweenOperator` for `useValueEditor`; `getValueSelectorUpdate` and `normalizeValueSelectorValue` for `useValueSelector`. Both hooks keep their signatures and return shapes; only the effect that applies the value reset remains React-specific.
...
- [#1079] `move`/`moveInPlace` no longer produce malformed queries when moving or cloning a rule/group to certain destinations within a query using independent combinators. Destination paths that identified a rule (an even index) could splice a rule into a combinator slot, breaking the rule/combinator alternation for everything after it—usually duplicating one rule and dropping another's combinator. Destination paths that identified a combinator (an odd index) are still interpreted as "insert after the preceding rule", and now resolve to the same location as the next even index, which also makes moves onto a combinator adjacent to the source a no-op instead of a corruption.
- [#1079] `move`/`moveInPlace` and `group`/`groupInPlace` no longer throw when the destination is inside the subtree being moved. The operation now aborts with `"destination-not-found"`.
- [#1079] `update`/`updateInPlace` no longer throw when setting `"combinator"` at a path that does not resolve (or whose parent is a rule) in a query using independent combinators. The operation now aborts with `"target-not-found"`.
- [#1079] `remove`/`removeInPlace` now report `"target-not-found"` for an unresolvable path in a query using standard combinators.
...
- [#1059] New `@react-querybuilder/expr` package adding arithmetic/function expression support to rules—either side of a rule can hold a recursive expression tree (e.g. `price * quantity`), though the bundled UI builds a full tree only on the right-hand side; on the left-hand side it wraps the field in a function, where the field is the function's first (governing) argument and any further arguments are edited as nested expressions.
  ...
  - Functions—including the arithmetic operators `+ - * /`—are modeled with metadata (`ExpressionFunctionMeta`, an optional `label` and `arity`) kept separate from per-format serializers, mirroring the datetime package's format-specific processors. Ships with `defaultFunctionMeta` (`add`/`subtract`/`multiply`/`divide`, `min`/`max`, `mod`, `abs`, `upper`/`lower`), matching `defaultSQLSerializers`/`defaultParameterizedSerializers`/`defaultJsonLogicSerializers` maps, and a `mergeFunctionMeta` helper for extending the metadata. SQL serializers may vary by dialect preset (e.g. `min`/`max` emit `LEAST`/`GREATEST`, overridden to `MIN`/`MAX` for the `sqlite` preset).
  - `formatQuery` rule processors for every export format that can represent computed operands: "sql", "parameterized"/"parameterized_named", "jsonlogic", "cel", "spel", "cypher"/"gql", "sparql", "jsonata", "mongodb_query" (and the deprecated "mongodb"), "elasticsearch", "natural_language", "drizzle", "sequelize", and "tanstack_db". Use the ready-made `expressionRuleProcessor*` bound to the built-ins (e.g. `expressionRuleProcessorSQL`, `expressionRuleProcessorDrizzle`), or build a custom processor with the matching `getExpressionRuleProcessor*` factory, each accepting a serializer registry merged over that format's defaults. Expression rules support scalar comparisons, `null`/`notNull`, and `between`/`notBetween` (with expression bounds); rules without expressions, with an unsupported operator, or missing the required `context`, fall back to the stock processor. String-match operators (`contains`, `beginsWith`, `endsWith`, and their negations) support expression operands—an expression RHS (search term) and/or LHS—for every export format except "tanstack_db", each emitting that dialect's native construct ("tanstack_db" omits such rules since its `like` needs a static pattern, and "sequelize" omits a string-match rule whose search term is a bare field reference). The library-backed formats need the same `context` helpers their stock processors need (`drizzle`: `columns`/`drizzleOperators`; `sequelize`: `sequelizeOperators`/`sequelizeWhere`/`sequelizeLiteral`/`sequelizeCol`; `tanstack_db`: `tanStackDbOperators`/`_tanstackDbRefs`/`_tanstackDbPrimaryRef`). The default JsonLogic serializers emit stock operators except for `abs`/`upper`/`lower`, whose runtime implementations are exported as `jsonLogicExpressionOperators` for registration on your `json-logic-js` instance.
  ...
  - Expression trees can reference a named query parameter via a new `"parameter"` `ExpressionNode` (`{ kind: 'parameter', parameter: string }`), mirroring the core `"parameter"` value source. Every format serializes it consistently with core: "sql"/"parameterized"/"parameterized_named" (and "sequelize") emit a prefix-adjusted bind reference (`parameterized_named` registers the key with a `null` placeholder, respecting `paramsKeepPrefix`; positional "parameterized" pushes nothing), "cel"/"spel" emit a bare identifier, and all other formats emit the name as a literal. The bundled `ExpressionEditor` renders a `<select>` of the `parameters` passed through `QueryBuilderExpressions` (falling back to a text input when none are supplied), and `validateExpression` flags an empty parameter name.
  - [#1066] Import support (parsing): every parser whose export format can represent expressions gains a `getExpression` option—the inverse of that format's expression serializers—that converts arithmetic/function operands on either side of a comparison into `ExpressionNode` trees. Supply the ready-made `expressionParser*` from `@react-querybuilder/expr` (or build one with the matching `getExpressionParser*` factory, which takes an inverse registry merged over that format's `default*Inverse`). An expression right-hand side becomes the rule's `value` with `valueSource: "expression"`; an expression left-hand side sets `field: ""` and stores the tree in `lhs`; inclusive-`between`/`notBetween` bounds become a two-element `value`. Unknown operators/functions, arity mismatches, and unknown field leaves cause the rule to be dropped. Each parser also gets a new `@react-querybuilder/core/parse*` subpath re-exporting its internals so `@react-querybuilder/expr` can supply the handler without a runtime dependency cycle. Supported parsers are `parseSQL`, `parseJsonLogic`, `parseJSONata`, `parseMongoDB`, `parseCEL`, and `parseSpEL`, with format-specific nuances:
    - `parseSQL`: ambiguous SQL spellings collapse to a single function (`%`/`MOD` → `mod`, `LEAST`/`MIN` → `min`, `GREATEST`/`MAX` → `max`); operators with no mapping (`^ | & << >> DIV`) drop the rule. Its subpath additionally re-exports `evalSQLLiteralValue`.
    - `parseJsonLogic`: the stock arithmetic operators (`+ - * / %`), `min`/`max`, and the custom `abs`/`upper`/`lower` operations map back to their `fn` keys.
    - `parseJSONata`: the stock arithmetic operators (`+ - * / %`) and the built-in functions `$abs`/`$min`/`$max`/`$uppercase`/`$lowercase` map back to their `fn` keys.
    - `parseMongoDB`: only `$expr` operands are routed through the handler (bare field references and literals fall through to the stock logic); the aggregation operators `$add`/`$subtract`/`$multiply`/`$divide`/`$min`/`$max`/`$abs`/`$mod`/`$toUpper`/`$toLower` map back to their `fn` keys.
    - `parseCEL`: infix arithmetic (`+ - * / %`) and the emulated `min`/`max` conditional templates are recovered; `abs`/`upper`/`lower` are not invertible (the CEL grammar cannot parse the constructs their exporters emit).
    - `parseSpEL`: infix arithmetic (`+ - * / %`) plus method and function calls are recovered—instance methods (`.toUpperCase()`/`.toLowerCase()` → `upper`/`lower`, with the receiver as the sole argument), `T(...)` static calls (`T(java.lang.Math).abs/min/max(...)`), and bare function calls (`myFunc(a, b)`). The `SpELInverse` registry accordingly gains `functions` and `methods` maps alongside `operators`, so custom calls can be mapped too. Note that `min`/`max` are exported as nested binary calls, so re-importing three or more arguments yields nested two-argument calls rather than a single variadic call.
...
- [#1065] New `"parameter"` value source, letting a rule compare a field against a named query parameter (placeholder) instead of a literal value or another field. Enable it by including `"parameter"` in a field's `valueSources` (or the `getValueSources` prop); the rule's `value` holds the selected parameter name.
  - New `QueryBuilder` prop `getParameters`. When provided and returning a non-empty option list, the value editor renders a `<select>` of these parameter names (a multiselect for `in`/`notIn` operators); when nullish or empty, it falls back to a free-form text input. Names should be specified without a prefix (e.g. `[{ name: 'p1', label: 'Param 1' }]`).
  - `formatQuery` understands the `"parameter"` value source across formats: SQL emits the prefix-adjusted name inline (`f1 = :p1`), "parameterized" emits it inline without pushing to `params`, "parameterized_named" registers it as a `params` key with a `null` placeholder value (respecting `paramsKeepPrefix`; `null` rather than `undefined` so the key survives `JSON.stringify`), and "cel"/"spel"/"jsonlogic" treat it as an identifier/variable reference (other formats emit it as a literal). The existing `paramPrefix` option controls the emitted prefix, and stored names already carrying the prefix are left as-is. String-match operators (`contains`/`beginsWith`/`endsWith` and their negations) concatenate the `%` wildcards around the bind reference (e.g. `firstName like :p1 || '%'`), keeping the wildcards out of the bound value; `in`/`notIn` and `between`/`notBetween` emit one reference per value.
  ...
  - [#1066] `parseSQL` gains a `parseParameters` option (`true` or `{ prefix?: string | string[]; positional?: boolean }`, default prefix `":"` with positional placeholders enabled)—the inverse of the export `"parameter"` handling—that recognizes bind placeholders and imports them as the `"parameter"` value source. Named placeholders (`:p1`) use the name; positional placeholders (`?`) become a 1-based ordinal name.
...
- [#1021] In `@react-querybuilder/dnd`, `react-dnd` is now an _optional_ peer dependency (it was previously required).
