...
- [#1078]/[#1079] [`QueryManager`](https://react-querybuilder.js.org/docs/utils/query-management#query-manager), a stateful, chainable wrapper around the [query tools](https://react-querybuilder.js.org/docs/utils/query-management#query-tools). Each method takes the same arguments as its query tool counterpart minus the leading `query` parameter. Adds `createRule`/`createRuleGroup` factories, `validate`, and `format`. Accepts the same `fields`, `operators`, `combinators`, and `getDefault*` configuration as the `QueryBuilder` component. No React dependency.
  ...
  - Configuration accessors `getFields` and `getCombinators` (the normalized top-level option lists, needed to populate field and combinator selectors), `getFieldData`, `getOperators`, `getValueSources`, `getMatchModes`, `getValues`, and `getValueEditorType`, plus `getRuleContext(pathOrID)` which resolves all of them for a single rule (along with its validation result) or returns `null` if the target isn't a rule, and `getRuleGroupContext(pathOrID)` which does the same for a group (combinator resolution, combinator-based classname, independent-combinator detection, and validation result; note that a group has no field-level validator fallback). These share the new framework-agnostic `deriveRuleContext` utility with the `useRule` hook, so non-React implementations resolve a rule's configuration identically without reimplementing the precedence rules. (`QueryManager` has no `getInputType` option, so `inputType` reflects only a field's own property.)
  - Framework-agnostic classname utilities `deriveRuleClassNames`, `deriveRuleOuterClassName`, `deriveRuleGroupClassNames`, and `deriveRuleGroupOuterClassName`, now used by the `useRule` and `useRuleGroup` hooks. Composition is declared as data—each derived classname names its contributing `controlClassnames` keys and any state-dependent classes—so conditional elements like the rule wrapper and group header go through the same path as everything else and any implementation produces identical `class` attributes. (A group's wrapper reflects fewer states than a rule's—`dndOver`, `dndCopy`, `dndDropNotAllowed`, and `hasSubQuery` are rule-only—so the two condition sets are declared separately rather than shared.)
  - Options `resetOnFieldChange` (default `true`), `resetOnOperatorChange` (default `false`), `maxLevels`, `respectDisabled` (default `true`), `queryDisabled`, `getInputType`, and `getSubQueryBuilderProps`, closing the gap with the corresponding `QueryBuilder` props. `getRuleContext` now reports `inputType` and `subQueryBuilderProps` accordingly.
  - The option-resolution precedence rules are now standalone core functions—`resolveOperatorList`, `resolveDefaultOperator`, `resolveValueEditorType`, `resolveValueList`, `createRule`, and `createRuleGroup`—shared by `useQueryBuilderSetup` and `QueryManager` instead of each maintaining its own copy. Two behaviors that had drifted between the two are now consistent, with the component's behavior taken as canonical: a `getDefaultField` function returning a falsy value leaves the first field in place (rather than producing an empty `field`), and `createRuleGroup` generates the group's `id` before any contained rule's.
  - The `useValueEditor` logic is now framework-agnostic: `getValueEditorReset`, `getMultiValueUpdate`, `coerceBigIntValue`, `coerceInputType`, and `isBetweenOperator`. The hook keeps its signature and return shape; only the effect that applies the reset remains React-specific. `deriveRuleClassName` composes a single rule classname, replacing the hand-rolled `valueListItem` class the hook previously built itself.
  - `useValueSelector`'s logic is likewise available as `getValueSelectorUpdate` and `normalizeValueSelectorValue`, and the controlled/uncontrolled precedence a query builder applies to decide what to render is available as `resolveCandidateQuery`.
  - `createQueryActions` builds the six mutations a query builder performs (`addRule`, `addGroup`, `propChange`, `removeRuleOrGroup`, `moveRule`, `groupRule`) as pure functions of the current query, each returning the next query or `undefined` when aborted. It owns the policy surrounding the query tools—disabled gating, the confirmation callback protocol (including the replacement-node and replacement-query return values), `maxLevels`, and `LogType` events—so `useQueryBuilderSchema` now supplies only the Redux storage around it.
  - `derivePathInfo` utility computes the path and disabled state of each child of a group; `usePathsMemo` now wraps it.
  - Inspection methods `isIC()`, `signatureOf(other)`, `diagnostics()` (shorthand for `format('diagnostics')`), and `toJSON()` (so `JSON.stringify(manager)` matches `JSON.stringify(manager.getQuery())`).
  ...
  - `useQueryManager` hook subscribes to a `QueryManager` and returns a `[query, manager]` tuple, re-rendering when the query changes. Pass an existing manager, or a query and options to have one created on the first render (and never recreated). Unlike the other hooks, it does not use Redux and does not need to be rendered beneath a `QueryBuilder`, so it suits headless or fully custom interfaces; queries managed this way are correspondingly invisible to `useQueryBuilderQuery`, `useQueryBuilderSelector`, `useQueryBuilderHistory`, and `getDispatchQueryById`.
- [#1078] Query tools accept opt-in guard options that block a mutation before it is attempted, reported through `onAbort` with reasons `"target-disabled"`, `"parent-disabled"`, and `"max-levels-exceeded"`:
  - `respectDisabled` aborts when the target (or, for `add`/`insert`, the parent) is disabled, either directly or by descending from a disabled group. Since `disabled` is a property of the query itself, honoring it is a matter of data integrity—a query saved with a locked rule stays locked when reloaded. Updating a node's own `disabled` property is always permitted, since that is the only way to re-enable it.
  - `queryDisabled` aborts every mutation, as though the whole query were disabled.
  - `maxLevels` caps the depth at which a group may be added. Rules are unaffected.
  - All three default to off, preserving existing behavior of the standalone tools. `QueryManager` enables `respectDisabled` by default, matching the `QueryBuilder` component. `getGuardAbortReason` and `exceedsMaxLevels` are exported so a UI layer can apply the same rules before running its own logic.
