...
- `@react-querybuilder/core`: `optionsEqual(a, b)` reports whether two `QueryManagerOptions` objects describe the same configuration, and `valuesEqual(a, b)` is the underlying comparison—arrays and plain objects by value (descending into nested objects like `history` and `translations`), everything else, functions included, by identity. `QueryManager#reconfigure` uses it to gate itself; it is exported for adapters that want to decide whether to call `reconfigure` at all.
- `@react-querybuilder/core`: `SubscriptionChange`, the payload type passed to `QueryManager#subscribe` listeners.
